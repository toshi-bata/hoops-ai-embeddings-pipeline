...
- **g6.8xlarge**(デフォルト)のUbuntu 24.04 LTSインスタンス。user-dataで以下をブートストラップ:
  ...
  - 2つのPython venv、`~/CPU1.1/.venv`(CPU専用torch)と`~/GPU1.1/.venv`(CUDA 13.0のtorch)
...
- **HOOPS AI SDK(`hoops_ai`パッケージ)はインストールしない**――デプロイ時に短命なダウンロードURL(`sdkUrl`コンテキスト/環境変数`HOOPS_AI_SDK_URL`)を渡した場合を除く。ライセンス製品であるため、この公開リポジトリには一切組み込まない。
...
- Linux向けのライセンス済みHOOPS AI SDKビルドと、ライセンスキー
...
| `sdkUrl` | (なし) | HOOPS AI SDKアーカイブの署名付きダウンロードURL。指定するとSDKを自動インストールする。環境変数`HOOPS_AI_SDK_URL`でも指定可 |
| `hoopsAiLicense` | (なし) | ライセンスキー。インスタンス上の`~/bench/.env`に書き込まれる(`sdkUrl`と併用した場合のみ有効)。環境変数`HOOPS_AI_LICENSE`でも指定可 |
...
# 署名付きSDK URLは&を含むクエリ文字列なのでクォートする
...
bashの場合: `$env:AWS_PROFILE = "..."`のような行を`export AWS_PROFILE=...`/`export HOOPS_AI_SDK_URL='...'`等に置き換える。

`HOOPS_AI_SDK_URL`を省略した場合でも、インスタンスは両方のvenvとcloneされたリポジトリを持った状態で起動する――後で手動で`~/CPU1.1/.venv`と`~/GPU1.1/.venv`にSDKを`pip install`するだけでよい。

`userDataCausesReplacement: true`のため、`assets/user-data.sh`や`assets/install-sdk.sh`を編集して再デプロイすると**インスタンスが置き換わる**(`InstanceId`が変わる)。これによりブートストラップは常にクリーンな状態から再実行される。
...
├── assets/user-data.sh           # EC2ブートストラップ(ドライバ、Xvfb、venv、リポジトリclone)
├── assets/install-sdk.sh         # 任意のSDKダウンロード+インストール(sdkUrl指定時)
└── README.md
```
