...
ベンチマーク実行: 成功 11 件、失敗/スキップ 0 件。生データ: `results/results.csv`、各実行ログは `logs/`。
...
- **Step 1（エンコード）と Step 3（埋め込み + 索引化）はCPUバウンド** — GPUはアイドルで、GPUインストールの恩恵はない。スループットはワーカー数に応じて物理コア数付近まで向上し、その後は頭打ち。
