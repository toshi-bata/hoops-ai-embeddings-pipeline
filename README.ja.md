...
- **Linuxで`num_workers`を大きくする場合はオープンファイル数上限の引き上げが必要。** Step 3(埋め込み+索引化)は一度に多数のファイルを開く――各ワーカーがSIGNALチェックポイント・Zarrデータセットのチャンク・プールのパイプを保持する――ため、20〜40以上のワーカーでスイープすると既定のソフト上限`ulimit -n 1024`を超え、`OSError: [Errno 24] Too many open files`で失敗することがある(親プロセスがクリーンに終了せずワーカープールでハングして見えることもある)。重量級のStep 3を実行する前に、同じシェルでソフト上限を引き上げること:
  ```bash
  ulimit -n 1048576   # ハード上限まで。上限は `ulimit -Hn` で確認
  ```
  この設定は子プロセスに継承されるので、スイープの前に一度実行すれば十分。(Step 1のエンコードはワーカーごとにモデルをロードしないため影響を受けない。)
- **長時間のリモート実行には`tmux`(または`nohup`)を使う。** 重量級のフルスイープは数時間かかるため、SSH切断に耐える端末マルチプレクサ内で実行する:
  ```bash
  tmux new -s bench
  ulimit -n 1048576
  ./run_heavy_scaling.sh 2>&1 | tee run.log   # Ctrl-b d でデタッチ
  tmux attach -t bench                         # 後で再アタッチ
  tail -f run.log                              # あるいはログを追うだけ
  ```
...
| `num_workers`を大きくするとStep 3が`OSError: [Errno 24] Too many open files`で落ちる(あるいはワーカーがアイドルのまま出力が止まりハングして見える) | 多ワーカーの埋め込みスイープに対して既定の`ulimit -n 1024`ソフト上限が低すぎる(各ワーカーがチェックポイント・Zarrチャンク・プールのパイプを保持する)。実行前に同じシェルで引き上げる:`ulimit -n 1048576`(上限は`ulimit -Hn`)。子プロセスに継承される |
